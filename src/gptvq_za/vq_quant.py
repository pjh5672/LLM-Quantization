...
def kmeans_m_step_3_zero_aware(
    centroids: torch.Tensor,
    n_centroids: int,
    assignments: torch.LongTensor,
    X: torch.Tensor,
    ...
    crange = torch.arange(0, n_centroids).to(centroids.device)
    clusters_for_centroid = [None] * X.shape[0]

    for i in range(X.shape[0]):
        X_ = X[i]
        norms = (X_ ** 2).sum(dim=1)
        valid_idx = (norms > 1e-6)
        X_ = X_[valid_idx].unsqueeze(0)
        assignments_ = assignments[i][valid_idx].unsqueeze(0)

        # G x N x 1 == 1 x 1 x K --> G x N x K
        assignments_expanded = (assignments_.unsqueeze(-1) == crange.view(1, 1, -1)).to(X.dtype)

        if H_inv_diag is None:
            norm = 1.0 / torch.clip(assignments_expanded.sum(1), min=1)  # G x K
            clusters_for_centroid_ = torch.einsum("gnd,gnk,gk->gkd", X_, assignments_expanded, norm)
        else:
            H_inv_diag_ = H_inv_diag[0][valid_idx]

            norm = 1.0 / torch.clip(
                torch.einsum("gnk,nd->gkd", assignments_expanded, H_inv_diag_), min=1e-10
            )
            clusters_for_centroid_ = torch.einsum(
                "gnd,nd,gnk,gkd->gkd", X_, H_inv_diag_, assignments_expanded, norm
            )
        clusters_for_centroid[i] = clusters_for_centroid_
    centroids.copy_(torch.cat(clusters_for_centroid, dim=0))
...
def mahalanobis_init_zero_aware(X, n_centroids):
    """
    X: G x N x D
    centroids: G x K x D
    """
    centroids = [None] * X.shape[0]

    for i in range(X.shape[0]):
        X_ = X[i]
        norms = (X_ ** 2).sum(dim=1)
        valid_idx = (norms > 1e-6)
        centroids[i] = mahalanobis_init(X_[valid_idx].unsqueeze(0), n_centroids)
