...
class Llama:
    ...
    def text_completion(self, prompts: list[str], temperature: float = 0.6,
        ...
        prompt_tokens = [self.tokenizer.encode(prompt, out_type=int, 
                                               ...
                                               for prompt in prompts]  # add beginnig of sentence not end of sentence
    
        # check batch size
        batch_size = len(prompt_tokens)
        assert batch_size <= self.model_args.max_batch_size, \
            f"Batch size {batch_size} exceeds the maximum batch size {self.model_args.max_batch_size}"
        # check maximum prompt length
        max_prompt_len = max(len(prompt) for prompt in prompt_tokens)
        assert max_prompt_len <= self.model_args.max_seq_len, \
            f"Maximum prompt length {max_prompt_len} exceeds the maximum sequence length {self.model_args.max_seq_len}"
...
if __name__ == "__main__":
    ...
    llama = Llama.build(
        checkpoints_dir="./models/Llama-2-7b-quantized/",
