...
class Transformer(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.freqs_complex = precompute_theta_pos_frequencies(args.dim // args.n_heads, 
                                                              args.max_seq_len * 2, 
                                                              device=args.device)
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        ...
        # This model is good for inference not for training, because in training we need to process the multiple tokens
...
if __name__ == "__main__":
    freqs_complex = precompute_theta_pos_frequencies(head_dim=128, seq_len=256, device='cuda')
    print(freqs_complex)