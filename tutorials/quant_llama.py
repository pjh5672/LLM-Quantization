...
def quantize_model(checkpoints_dir: str, tokenizer_dir: str, 
                   save_dir: str, quantization_type: str, device: str = "cpu"):
    """
    ...
    print(f"Original model size: {get_model_size('./models/Llama-2-7b/') / (1024**3):.2f} GB")

    # For the quantized model
    print(f"Quantized model size: {get_model_size('./models/Llama-2-7b-quantized/') / (1024**3):.2f} GB")


if __name__ == "__main__":
    quantize_model(
        checkpoints_dir="./models/Llama-2-7b",
        tokenizer_dir="./models/Llama-2-7b/tokenizer.model",
        save_dir="./models/Llama-2-7b-quantized",
